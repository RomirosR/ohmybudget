...
def test_plan_crud_and_summary(auth_client):
    ...
    assert auth_client.delete(f"/api/plans/{plan_id}").status_code == 204
    assert auth_client.get("/api/plans").json() == []


def test_clone_next_month(auth_client):
    auth_client.post(
        "/api/plans",
        json={
            "year": 2026, "month": 1, "category": "Зарплата",
    ...
    r = auth_client.post("/api/plans/clone-next")
    assert r.status_code == 201
    created = r.json()
    assert len(created) == 1
    assert created[0]["month"] == 2
    assert created[0]["year"] == 2026


def test_clone_next_with_no_plans_returns_400(auth_client):
    assert auth_client.post("/api/plans/clone-next").status_code == 400


def test_opening_balance_upsert(auth_client):
    auth_client.post(
        "/api/plans",
        json={
...
def test_assets_total(auth_client):
    auth_client.post(
        "/api/assets",
    ...
    data = auth_client.get("/api/assets").json()
    assert data["total"] == 17000
    assert len(data["items"]) == 2


def test_investments_total_income(auth_client):
    auth_client.post(
        "/api/investments",
        json={
    ...
    data = auth_client.get("/api/investments").json()
