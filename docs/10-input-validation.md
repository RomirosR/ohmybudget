...
## Шаг 4 — Подсказка только при ошибке ввода (коммит: 1caa129)

**Дата:** 2026-06-08

**Что сделано:**
- Постоянная подсказка убрана — интерфейс не сдвигается.
- Сообщение появляется только если пользователь ввёл недопустимый символ (буква, лишний минус и т.п.).
- Всплывающая подсказка с `position: absolute` поверх контента; рамка поля краснеет; скрытие через 4 с или при blur.

**Как проверить:** в поле суммы набрать букву — краткое сообщение всплывает под полем, layout не прыгает.

## Шаг 5 — Скрытие подсказки при корректном вводе (коммит: c35723c)

**Дата:** 2026-06-08

**Что сделано:** после ошибочного символа подсказка гаснет сразу при следующем допустимом вводе (цифра и т.п.), не дожидаясь таймера.

**Как проверить:** ввести букву → подсказка; затем цифру → подсказка сразу исчезает.
